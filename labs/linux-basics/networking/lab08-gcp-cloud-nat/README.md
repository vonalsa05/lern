...
- [Что нужно](#что-нужно)
- [Задание 0: Настройка переменных](#задание-0-настройка-переменных)
- [Задание 1: Создание VPC и приватной подсети](#задание-1-создание-vpc-и-приватной-подсети)
- [Задание 2: Создание приватной VM](#задание-2-создание-приватной-vm)
- [Задание 3: Создание Cloud Router и Cloud NAT](#задание-3-создание-cloud-router-и-cloud-nat)
- [Задание 4: Проверка работы Cloud NAT](#задание-4-проверка-работы-cloud-nat)
- [Задание 5: Исследование параметров трансляции (NAT)](#задание-5-исследование-параметров-трансляции-nat)
- [Задание 6: Логирование и мониторинг Cloud NAT](#задание-6-логирование-и-мониторинг-cloud-nat)
- [Задание 7: Очистка ресурсов](#задание-7-очистка-ресурсов)
- [Чеклист выполнения](#checklist-выполнения)
- [Ответы на вопросы (для самопроверки)](#ответы-на-вопросы-для-самопроверки)
<!-- /TOC -->

> ⏱ время ~40 мин · сложность 3/5 · пререквизиты: базовый CLI, основы сетей, пройденная [lab01-routing-dnat](../lab01-routing-dnat/)

## Обзор лабораторной

В этой лабораторной вы построите типовую облачную архитектуру: приватные VM без публичных IP-адресов, которые выходят в интернет через Cloud NAT. Это стандартный паттерн безопасности в продакшн-инфраструктуре.

**Что вы сделаете:** создадите VPC с приватной подсетью, развернете VM без внешнего IP, убедитесь, что без NAT доступа в интернет нет, настроите Cloud Router и Cloud NAT, проверите исходящий доступ, исследуете поведение NAT: порты, IP, логи.

## Связь с теоретическим конспектом
Перед выполнением лабораторной работы рекомендуется ознакомиться с теорией о глобальных сетях и протоколах взаимодействия в [Теоретическом конспекте](file:///root/lern/labs/linux-basics/networking/abstract1/README.md).
...
> [!WARNING]
> **КРИТИЧЕСКИ ВАЖНО: ПРЕДУПРЕЖДЕНИЕ О СТОИМОСТИ!**
> Все создаваемые ресурсы в Google Cloud Platform (GCP) — виртуальные машины, Cloud Router, Cloud NAT и особенно **статические IP-адреса** — тарифицируются.
> Неиспользуемый, но зарезервированный статический IP-адрес стоит дороже, чем используемый!
> Обязательно выполните все шаги по очистке из **Задания 7** в конце лабораторной работы и убедитесь, что ресурсы действительно удалены.
...
Задайте переменные, чтобы использовать их во всех командах:
...
**Шаг 1.** Создайте VPC с отключённым автоматическим созданием подсетей:
...
**Шаг 2.** Создайте подсеть с приватным диапазоном:
...
**Шаг 3.** Создайте firewall-правило для SSH (через IAP — Identity-Aware Proxy):
...
**Шаг 1.** Создайте VM без внешнего IP:
...
**Шаг 2.** Подключитесь через IAP:

```bash
gcloud compute ssh nat-lab-vm --zone=$ZONE --tunnel-through-iap
```

**Шаг 3.** Проверьте, что внешнего IP нет:
...
**Шаг 4.** Попробуйте что-нибудь скачать:
...
> **Подсказка:** Проверьте маршруты VM: `ip route show`. Есть ли default gateway? Куда он ведёт? Что происходит с пакетом, когда он доходит до gateway VPC?
...
**Шаг 1.** Создайте Cloud NAT с автоматическим выделением IP:
...
```bash
# Подключитесь к VM (если отключился)
gcloud compute ssh nat-lab-vm --zone=$ZONE --tunnel-through-iap
...
**Шаг 3.** Узнай, через какой IP вы выходишь, и сравните с NAT Gateway:
...
**Шаг 2.** Подожди 2–3 минуты и посмотрите логи (в Cloud Shell):
...
> ⚠️ **Важно:** удалите все ресурсы, чтобы не платить за них. Удаляйте в обратном порядке (зависимости).
...
### Проверка полной очистки ресурсов
После удаления ресурсов обязательно убедитесь, что в вашем проекте не осталось "забытых" элементов. Запустите следующие команды проверки:

```bash
# Проверить отсутствие VM
gcloud compute instances list --filter="name=nat-lab-vm"

# Проверить отсутствие маршрутизаторов
gcloud compute routers list --filter="name=nat-lab-router"

# Проверить отсутствие статических IP
gcloud compute addresses list --filter="name=nat-lab-static-ip"

# Проверить отсутствие VPC
gcloud compute networks list --filter="name=nat-lab-vpc"
```
Каждая из этих команд должна вернуть пустой вывод (кроме возможных системных сообщений). Если какая-то команда возвращает ресурс, значит, очистка не завершилась или произошла ошибка.
...
3. Зачем нужен Cloud Router, если Cloud NAT не использует виртуальные машины для маршрутизации трафика?
...
**Задание 1 — subnet-mode=custom vs auto:** В auto-режиме GCP создаёт подсеть в каждом регионе с предопределёнными диапазонами. Custom даёт полный контроль: вы решаешь, в каких регионах будут подсети и с какими CIDR. Для продакшна всегда custom — иначе получишь пересечение адресов при VPC peering.
